...
| `azd up` échoue sur `foundryProject` avec `BadRequest: ... To create projects, you must enable a managed identity on your resource` (et un message générique "A resource with this name already exists or is in a conflicting state") | Le compte `openAi` (`kind: AIServices`) n'a pas d'identité managée alors que `allowProjectManagement: true` en requiert une (DECISIONS.md #26) | Déjà corrigé dans `infra/resources.bicep` (`identity: { type: 'SystemAssigned' }` sur `openAi`) ; relancer `azd up`. Si l'erreur "already exists" persiste, vérifier dans le portail (compte `aoai-${resourceToken}` → onglet *Projects*) qu'il n'y a pas de `proj-${resourceToken}` resté en état `Failed`, et le supprimer avant de relancer |
| `azd up` échoue sur `Azure AI Services: aoai-${resourceToken}` avec `FailedIdentityOperation: ... Status: 'Conflict'. ... is new, but resource already exists. This may be due to a pending delete operation, try again later` | Conflit transitoire côté fournisseur d'identités managées (MSI) lors de l'ajout d'une identité `SystemAssigned` à un compte `aoai-${resourceToken}` déjà existant (créé par un `azd up` précédent avant ce changement) | Le bicep est correct (identité requise par `allowProjectManagement: true`, DECISIONS.md #26) — c'est une erreur Azure transitoire. Solution rapide : portail → ressource `aoai-${resourceToken}` → **Identité** → **Affectée par le système** → **Activé** → Enregistrer, puis relancer `azd provision`/`azd up` (le bicep constatera que l'identité correspond déjà et pourra créer `foundryProject`). Sinon, attendre quelques minutes (le temps que l'opération en attente côté Azure se termine) puis relancer `azd up` |
